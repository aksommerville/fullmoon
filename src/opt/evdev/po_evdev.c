...
static int po_evdev_device_add_axis(
  ...
  if ((absinfo->maximum>=absinfo->minimum+1)&&(absinfo->value==absinfo->minimum)) {
    ...
    axis->axis=(code&1)?FMN_BUTTON_A:FMN_BUTTON_B;
    device->axisc++;
    return 0;
  }
...
static const struct po_evdev_button_map {
  ...
  {0x0079,0x0011,0x0120,FMN_BUTTON_B}, // east (north naturally maps to A)
  {0x0079,0x0011,0x0121,FMN_BUTTON_A}, // south
  {0x0079,0x0011,0x0123,FMN_BUTTON_B}, // west
  ...
  {0x2dc8,0x5006,0x0131,FMN_BUTTON_A}, // B
  {0x2dc8,0x5006,0x0134,FMN_BUTTON_B}, // Y
  {0x2dc8,0x5006,0x0130,FMN_BUTTON_A}, // A
  {0x2dc8,0x5006,0x0133,FMN_BUTTON_B}, // X
  {0x2dc8,0x5006,0x013b,BUTTON_QUIT}, // start
  
  //2dc8:6001 '8Bitdo SF30 Pro   8Bitdo SN30 Pro' (gray)
  {0x2dc8,0x6001,0x0131,FMN_BUTTON_A}, // south
  {0x2dc8,0x6001,0x0134,FMN_BUTTON_B}, // west
  {0x2dc8,0x6001,0x013e,BUTTON_QUIT}, // right plunger

  // 8BitDo Tech Co., Ltd. 8BitDo Zero 2 gamepad (pink)
  {0x2dc8,0x9018,0x0131,FMN_BUTTON_A}, // south
  {0x2dc8,0x9018,0x0134,FMN_BUTTON_B}, // west
...
static int po_evdev_event_unconfigured(
  ...
  switch (event->type) {
        ...
        if (axis) switch (axis->mode) {
          ...
          case PO_EVDEV_AXIS_MODE_TWOWAY: {
              ...
              if (v!=axis->v) {
                ...
                if (axis->axis==PO_EVDEV_AXIS_HORZ) {
                  if ((v<=0)&&(device->state&FMN_BUTTON_RIGHT)) {
                    device->state&=~FMN_BUTTON_RIGHT;
                    if (evdev->cb_button(evdev,FMN_BUTTON_RIGHT,0)<0) return -1;
                  }
                  if ((v>=0)&&(device->state&FMN_BUTTON_LEFT)) {
                    device->state&=~FMN_BUTTON_LEFT;
                    if (evdev->cb_button(evdev,FMN_BUTTON_LEFT,0)<0) return -1;
                  }
                  if ((v<0)&&!(device->state&FMN_BUTTON_LEFT)) {
                    device->state|=FMN_BUTTON_LEFT;
                    if (evdev->cb_button(evdev,FMN_BUTTON_LEFT,1)<0) return -1;
                  }
                  if ((v>0)&&!(device->state&FMN_BUTTON_RIGHT)) {
                    device->state|=FMN_BUTTON_RIGHT;
                    if (evdev->cb_button(evdev,FMN_BUTTON_RIGHT,1)<0) return -1;
                  }
                } else {
                  if ((v<=0)&&(device->state&FMN_BUTTON_DOWN)) {
                    device->state&=~FMN_BUTTON_DOWN;
                    if (evdev->cb_button(evdev,FMN_BUTTON_DOWN,0)<0) return -1;
                  }
                  if ((v>=0)&&(device->state&FMN_BUTTON_UP)) {
                    device->state&=~FMN_BUTTON_UP;
                    if (evdev->cb_button(evdev,FMN_BUTTON_UP,0)<0) return -1;
                  }
                  if ((v<0)&&!(device->state&FMN_BUTTON_UP)) {
                    device->state|=FMN_BUTTON_UP;
                    if (evdev->cb_button(evdev,FMN_BUTTON_UP,1)<0) return -1;
                  }
                  if ((v>0)&&!(device->state&FMN_BUTTON_DOWN)) {
                    device->state|=FMN_BUTTON_DOWN;
                    if (evdev->cb_button(evdev,FMN_BUTTON_DOWN,1)<0) return -1;
                  }
                }
              }
            } break;
          ...
          case PO_EVDEV_AXIS_MODE_HAT: {
              ...
              if (v!=axis->v) {
                uint16_t nstate=0;
                switch (axis->v=v) {
                  case 0: nstate=FMN_BUTTON_UP; break;
                  case 1: nstate=FMN_BUTTON_UP|FMN_BUTTON_RIGHT; break;
                  case 2: nstate=FMN_BUTTON_RIGHT; break;
                  case 3: nstate=FMN_BUTTON_RIGHT|FMN_BUTTON_DOWN; break;
                  case 4: nstate=FMN_BUTTON_DOWN; break;
                  case 5: nstate=FMN_BUTTON_DOWN|FMN_BUTTON_LEFT; break;
                  case 6: nstate=FMN_BUTTON_LEFT; break;
                  case 7: nstate=FMN_BUTTON_LEFT|FMN_BUTTON_UP; break;
                }
                uint16_t ostate=device->state&(FMN_BUTTON_UP|FMN_BUTTON_DOWN|FMN_BUTTON_LEFT|FMN_BUTTON_RIGHT);
                device->state=(device->state&(FMN_BUTTON_A|FMN_BUTTON_B))|nstate;
                #define _(tag) \
                  if ((nstate&FMN_BUTTON_##tag)&&!(ostate&FMN_BUTTON_##tag)) { \
                    if (evdev->cb_button(evdev,FMN_BUTTON_##tag,1)<0) return -1; \
                  } else if (!(nstate&FMN_BUTTON_##tag)&&(ostate&FMN_BUTTON_##tag)) { \
                    if (evdev->cb_button(evdev,FMN_BUTTON_##tag,0)<0) return -1; \
                  }
                _(UP)
                _(DOWN)
                _(LEFT)
    ...
    case EV_KEY: switch (event->code) {
        ...
        case KEY_UP: return po_evdev_key(evdev,device,FMN_BUTTON_UP,event->value);
        case KEY_DOWN: return po_evdev_key(evdev,device,FMN_BUTTON_DOWN,event->value);
        case KEY_LEFT: return po_evdev_key(evdev,device,FMN_BUTTON_LEFT,event->value);
        case KEY_RIGHT: return po_evdev_key(evdev,device,FMN_BUTTON_RIGHT,event->value);
        case KEY_Z: return po_evdev_key(evdev,device,FMN_BUTTON_A,event->value);
        case KEY_X: return po_evdev_key(evdev,device,FMN_BUTTON_B,event->value);
  ...
  if (event->type==EV_ABS) {
    ...
    switch (map->dstvalue) {
      case -1: switch (map->usage) {
          case PO_EVDEV_USAGE_HORZ: device->state&=~FMN_BUTTON_LEFT; if (evdev->cb_button(evdev,FMN_BUTTON_LEFT,0)<0) return -1; break;
          case PO_EVDEV_USAGE_VERT: device->state&=~FMN_BUTTON_UP; if (evdev->cb_button(evdev,FMN_BUTTON_UP,0)<0) return -1; break;
        } break;
      case 1: switch (map->usage) {
          case PO_EVDEV_USAGE_HORZ: device->state&=~FMN_BUTTON_RIGHT; if (evdev->cb_button(evdev,FMN_BUTTON_RIGHT,0)<0) return -1; break;
          case PO_EVDEV_USAGE_VERT: device->state&=~FMN_BUTTON_DOWN; if (evdev->cb_button(evdev,FMN_BUTTON_DOWN,0)<0) return -1; break;
        } break;
    }
    
    switch (map->dstvalue=dstvalue) {
      case -1: switch (map->usage) {
          case PO_EVDEV_USAGE_HORZ: device->state|=FMN_BUTTON_LEFT; if (evdev->cb_button(evdev,FMN_BUTTON_LEFT,1)<0) return -1; break;
          case PO_EVDEV_USAGE_VERT: device->state|=FMN_BUTTON_UP; if (evdev->cb_button(evdev,FMN_BUTTON_UP,1)<0) return -1; break;
        } break;
      case 1: switch (map->usage) {
          case PO_EVDEV_USAGE_HORZ: device->state|=FMN_BUTTON_RIGHT; if (evdev->cb_button(evdev,FMN_BUTTON_RIGHT,1)<0) return -1; break;
          case PO_EVDEV_USAGE_VERT: device->state|=FMN_BUTTON_DOWN; if (evdev->cb_button(evdev,FMN_BUTTON_DOWN,1)<0) return -1; break;
        } break;
    }
    ...
    switch (map->usage) {
      case PO_EVDEV_USAGE_LEFT: if (evdev->cb_button(evdev,FMN_BUTTON_LEFT,dstvalue)<0) return -1; break;
      case PO_EVDEV_USAGE_RIGHT: if (evdev->cb_button(evdev,FMN_BUTTON_RIGHT,dstvalue)<0) return -1; break;
      case PO_EVDEV_USAGE_UP: if (evdev->cb_button(evdev,FMN_BUTTON_UP,dstvalue)<0) return -1; break;
      case PO_EVDEV_USAGE_DOWN: if (evdev->cb_button(evdev,FMN_BUTTON_DOWN,dstvalue)<0) return -1; break;
      case PO_EVDEV_USAGE_A: if (evdev->cb_button(evdev,FMN_BUTTON_A,dstvalue)<0) return -1; break;
      case PO_EVDEV_USAGE_B: if (evdev->cb_button(evdev,FMN_BUTTON_B,dstvalue)<0) return -1; break;
